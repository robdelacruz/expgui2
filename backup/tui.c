...
#define COLOR_HIGHLIGHT_FG TB_BLACK
#define COLOR_HIGHLIGHT_BG TB_WHITE
...
static void draw_box(uint x, uint y, uint width, uint height);
...
void tui_start(sqlite3 *db, char *dbfile) {
    ...
    draw_box(screenrect.x, screenrect.y, screenrect.width, screenrect.height);
    listbox_draw(lb);
    tb_present();
...
static void draw_box(uint x, uint y, uint width, uint height) {
    for (int i=x; i < x+width; i++) {
        if (i == x)
            tb_print(i,y, COLOR_FG, COLOR_BG, "┌");
        else if (i == x+width-1)
            tb_print(i,y, COLOR_FG, COLOR_BG, "┐");
        else
            tb_print(i,y, COLOR_FG, COLOR_BG, "─");
    ...
    for (int i=y+1; i < y+height-1; i++) {
        tb_print(x,i, COLOR_FG, COLOR_BG, "│");
        tb_print(x+width-1,i, COLOR_FG, COLOR_BG, "│");
    }
    for (int i=x; i < x+width; i++) {
        if (i == x)
            tb_print(i,y+height-1, COLOR_FG, COLOR_BG, "└");
        else if (i == x+width-1)
            tb_print(i,y+height-1, COLOR_FG, COLOR_BG, "┘");
        else
            tb_print(i,y+height-1, COLOR_FG, COLOR_BG, "─");
...
static void listbox_draw(listbox_t *lb) {
    ...
    for (int i = lb->scrollpos; i < lb->rows->len; i++) {
        ...
        if (i == lb->sel)
            (*draw_row)(lb, irow, COLOR_HIGHLIGHT_FG, COLOR_HIGHLIGHT_BG);
        else
